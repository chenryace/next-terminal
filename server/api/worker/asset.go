...
func (api WorkAssetApi) PagingEndpoint(c echo.Context) error {
	...
	items, total, err := service.AccountService.FindMyAssetPaging(pageIndex, pageSize, name, protocol, tags, account.ID, order, field)
	if err != nil {
		return err
	}
	for i := range items {
...
func (api WorkAssetApi) TagsEndpoint(c echo.Context) (err error) {
	account, _ := api.GetCurrentAccount(c)
	var items []string
	if items, err = service.AccountService.FindMyAssetTags(context.TODO(), account.ID); err != nil {
		return err
	}
