...
func (r authorisedRepository) FindAssetPage(c context.Context, pageIndex, pageSize int, assetName, userId, userGroupId string) (o []dto.AssetPageForAuthorised, total int64, err error) {
	db := r.GetDB(c).Table("assets").
		Select("authorised.id, authorised.created, assets.id as asset_id, assets.name as asset_name, command_filters.id as command_filter_id, command_filters.name as command_filter_name, strategies.id as strategy_id, strategies.name as strategy_name ").
		Joins("left join authorised on authorised.asset_id = assets.id").
		Joins("left join command_filters on command_filters.id = authorised.command_filter_id").
...
func (r authorisedRepository) FindUserPage(c context.Context, pageIndex, pageSize int, userName, assetId string) (o []dto.UserPageForAuthorised, total int64, err error) {
	db := r.GetDB(c).Table("users").
		Select("authorised.id, authorised.created, users.id as user_id, users.nickname as user_name, command_filters.id as command_filter_id, command_filters.name as command_filter_name, strategies.id as strategy_id, strategies.name as strategy_name ").
...
func (r authorisedRepository) FindUserGroupPage(c context.Context, pageIndex, pageSize int, userName, assetId string) (o []dto.UserGroupPageForAuthorised, total int64, err error) {
	db := r.GetDB(c).Table("user_groups").
		Select("authorised.id, authorised.created, user_groups.id as user_group_id, user_groups.name as user_group_name, command_filters.id as command_filter_id, command_filters.name as command_filter_name, strategies.id as strategy_id, strategies.name as strategy_name ").
